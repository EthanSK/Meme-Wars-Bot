...
// $fb->setExtendedAccessToken($access_token);
//  echo ("<pre>");
...
foreach($pages as $keyPage)
	...
	if ($keyPage['name'] == 'Meme Wars in the Comments')
		...
		$timeAcceptedBetweenMyMessageReply = 18000;
		...
		print_r($myPosts[0]);
		echo ("</pre>");
		// $testGet = $fb->get('/1374492432644995/comments?limit=100000&fields=message,attachment,from,message_tags,like_count,likes.limit(1000000),type,reactions.limit(1000000)', $keyPage['access_token']);
		// $testGet = $testGet->getGraphEdge()->asArray();


		$commentsOnLatestPost = $fb->get('/' . $myPosts[0]['id'] . '/comments?limit=10000&fields=message,attachment,from,message_tags,like_count,likes,type,reactions.limit(10000)', $keyPage['access_token']);
		$commentsOnLatestPost = $commentsOnLatestPost->getGraphEdge()->asArray();


				foreach ($commentsOnLatestPost as $key => $value) {
					$testGet[$key]['reaction_count'] = count($value['reactions']);
				}

						 echo ("<pre>");
						 echo 'comments on latest post  ';
		...
		foreach($commentsOnLatestPost as $key => $row)
			...
			$likeCount[$key] = $row['reaction_count'];
			}


				function sortById($x, $y) {
    return $y['reaction_count'] - $x['reaction_count'];
}
		...
		foreach($commentsOnLatestPost as $key => $value)
			...
			foreach($commentsOnLatestPost as $key2 => $value2)
				...
				if ($value2['from']['id'] == $value['from']['id'])
					...
					//likes on this comment is now reactions because fucking zucc is a pile of nigger
				$likesOnThisComment = $fb->get('/' . $value2['id'] . '/reactions?limit=10000', $keyPage['access_token']);
	 ...
	 echo ("<pre>");
	   print_r('there are no likes so $number of likes required is set to 0. Actually no, i will get the bot to like a meme. All around me are familiar faces. fuck this doesnt work as well due to blocc so ill have to just auto selecet a manvan, fredman, or cakis post.');
		  echo ("</pre>");
			...
			echo ' has '. $value['reaction_count']." likes and is the person that made this comment has liked this many other posts: ". $numberOfLikesThisPersonHasMade;
			echo ("</pre>");
				...
				if ($numberOfLikesThisPersonHasMade >= count($arrayOfTotalCommentsByPersonUnique) - 1)
					{
						foreach ($repliesOnThisComment as $keyrepliesoncomment => $valuerepliesoncomment) {
							...
							if(strpos($valuerepliesoncomment['message'], "eligible") !== false && $valuerepliesoncomment['from']['id'] == $keyPage['id']){
								echo 'this person didnt like enough memes';
								$didNotVote = true;

							}else {echo 'this did like enough memes';}

						}
							if (!$isZuccWorthy && !$didNotVote){
				...
				if ($numberOfLikesThisPersonHasMade >= $numberOfLikesRequiredToBeEligible)
					{
						...
						foreach ($repliesOnThisComment as $keyrepliesoncomment => $valuerepliesoncomment) {
							 echo ("<pre>");
							 echo ('replies on comment');
							   print_r($valuerepliesoncomment);
								  echo ("</pre>");
							...
							if(strpos($valuerepliesoncomment['message'], "zuccworthy") !== false && $valuerepliesoncomment['from']['id'] == $keyPage['id']){
								echo 'this person is zuccworthy based on the current reply being scanned  ';
								$isZuccWorthy = true;

							}else {echo 'this person is not zuccworthy based on the current reply being scanned';}
					...
					if ($enablePosting)
						...
						foreach($repliesOnThisPost as $keyrep => $valuerep)
							...
							if ($value['from']['id'] == '118862731965062' || $value['from']['id'] == '154227091749617'|| $value['from']['id'] == '106094306588165'){
								//$stopReply = true;
								//stop reply does not mean it stops it from not winnig, it just stops the reply. THerefore, the fake accounts will be deemed ineligible even if they are not told so.
								//Actually ,thinking about it, it is probably a good thing for that to happen, so people can see that no matter how many likes you get, you need to like other ones.
								//maybe ill make it so the fake accounts sometimes like other comments
							}
...
//fake accounts
		...
		{
			$noOneCommentedSoIHaveToComment = true;
		...
		if (!empty($arrayWithNameAndCommentOfWinner['imageURL']))
			...
			if (!empty($arrayWithNameAndCommentOfWinner['message']))
				...
				if ($typeOfProfile == 'page')
					{
					$toPost = $toPost + array(
						'message' => "Congratulations, https://facebook.com/" . $arrayOfPeopleIDsEligible[0] . ", your meme won!\n\n Your meme: " . $arrayWithNameAndCommentOfWinner['message']
					);
					}
				  else
					{
		...
		if (!empty($arrayWithNameAndCommentOfWinner['videoURL']))
			...
			if (!empty($arrayWithNameAndCommentOfWinner['message']))
				...
				if ($typeOfProfile == 'page')
					{
					$toPost = $toPost + array(
						'description' => "Congratulations, https://facebook.com/" . $arrayOfPeopleIDsEligible[0] . ", your meme won!\n\n Your meme: " . $arrayWithNameAndCommentOfWinner['message']
					);
					}
				  else
					{
		...
		$toComment = array(
			...
			'message' => "If you want a chance of winning, simply comment your best meme on the latest post, and like at least $numberOfLikesRequiredToBeEligible other memes you think are worthy of winning. You can comment photo or video memes.\n\nHere is the link to the original comment: " . $linkToComment
		);
...
$winnerImageOrVideo = $arrayWithNameAndCommentOfWinner['videoURL'];
	...
	$messages = $fb->get('/' . $keyPage['id'] . '/conversations?limit=100000&fields=snippet,unread_count,senders,can_reply', $keyPage['access_token']);
	$messages = $messages->getGraphEdge()->asArray();
	$wasMessageMeCalledBecauseTooMuchTimePassed =false;
...
//if it's my convo
    if ($valuemes['senders'][0]['name'] == 'Ethan Sarif-Kattan' && $valuemes['can_reply'] == 1)
      {
        $individualMessage = $fb->get('/' . $valuemes['id'] . '/messages?limit=10&fields=from,message,created_time', $keyPage['access_token']);
        $individualMessage = $individualMessage->getGraphEdge()->asArray();
//	basically if im a lazy fuck and dont respond spam me until i do
				if ($individualMessage[0]['from']['id'] == $keyPage['id'] && date_timestamp_get($individualMessage[0]['created_time']) + $timeAcceptedBetweenMyMessageReply < time()){
					require "memewarsmessagemecheck.php";
					$wasMessageMeCalledBecauseTooMuchTimePassed = true;
				}
				//  echo ("<pre>");
				//    print_r($individualMessage);
				// 	  echo ("</pre>");
				// 		 echo ("<pre>");
				// 		 echo 'timestamp of most recent message: ';
				// 		   print_r(date_timestamp_get($individualMessage[0]['created_time']));
				// 			  echo ("</pre>");
	...
	if ($enablePosting && !$noOneCommentedSoIHaveToComment){
		echo 'requiring memewars message me check';
